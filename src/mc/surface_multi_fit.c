...
// Copyright Adam Hogan 2016-2017 - whatever license this thing comes with
    ...
    for (i=0;i<params->nParams;i++) {
        ...
        if (fit) {
            if ( params->epsilon[i] > 0.0 ) {
                if ( system->polarvdw )
                    params->epsilon[i] += params->epsilon[i] * ( scale_epsilon*(0.5 - get_rand()) );
                else
                    params->epsilon[i] += scale_epsilon*(0.5 - get_rand());
            ...
            if  ( system->polarvdw || system->disp_expansion_mbvdw ) {
                if( params->omega[i] > 0.0 )
                    params->omega[i] += scale_omega*(0.5 - get_rand());
                ...
                if( system->cdvdw_exp_repulsion ) {
                    if ( params->sigma[i] > 0.0 ) //scale_sigma given as percentage when exp_rep is on
                        params->sigma[i] += params->sigma[i] *( scale_sigma*(0.5 - get_rand()) );
            ...
            //otherwise adjust sigma
                ...
                if ( params->sigma[i] > 0.0 )
                    params->sigma[i] += scale_sigma*(0.5 - get_rand());
                if( params->sigma[i] < 0.0 ) params->sigma[i] = params->last_sigma[i];
            }
...
double calc_multi_error(system_t* system, multiConfigData_t* configs) {
	double total_error = 0.0, model, weight;
	...
	for (i=0;i<configs->nConfigs;i++) {
		...
		if (configs->fitEnergy[i] == MAXVALUE)
			configs->lastFitEnergy[i] = current_energy;
		configs->fitEnergy[i] = current_energy;
		...
		if ( system->fit_boltzmann_weight == 0 ) {
			...
			abInitio = min(max_energy, abInitio);
			model    = min(max_energy, current_energy);
			weight   = exp(kweight*(max_energy-abInitio)/max_energy);
		} else {
			model = current_energy;
			weight = exp(kweight*(-abInitio)/max_energy);
		}

		total_error += weight * (model-abInitio) * (model-abInitio);
	}
...
int surface_multi_fit (system_t *system) {
	...
	//calloc(1000000000,sizeof(char)); // fuck you
	return 0;
}

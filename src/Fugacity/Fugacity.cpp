...
double Fugacity::h2_fugacity(double temperature, double pressure) {

	if((temperature == 77.0) && (pressure <= 200.0)) {

		Output::out("INPUT: fugacity calculation using Zhou function\n");
		return h2_fugacity_zhou(temperature, pressure);
	}  
	else if(temperature >= 273.15) {

		Output::out("INPUT: fugacity calculation using Shaw function\n");
		return h2_fugacity_shaw(temperature, pressure);

	} else {

		Output::out("INPUT: fugacity calculation using BACK EoS\n");
...
double Fugacity::h2_fugacity_back(double temperature, double pressure) 
{

	char   linebuf[maxLine];
	double fugacity_coefficient, 
			fugacity,
			comp_factor;
...
double Fugacity::h2_comp_back(double temperature, double pressure) 
{
	// Constants
	constexpr double BACK_H2_ALPHA =  1.033;
	constexpr double BACK_H2_U0    = 38.488;
	constexpr double BACK_H2_V00   =  9.746;
	constexpr double BACK_H2_N     =  0.00;
	constexpr double BACK_C        =  0.12;
	const     int    BACK_MAX_M    =  9;
	const     int    BACK_MAX_N    =  4;
...
double Fugacity::h2_fugacity_shaw(double temperature, double pressure) {
...
double Fugacity::h2_fugacity_zhou(double temperature, double pressure) {
...
double Fugacity::ch4_fugacity(double temperature, double pressure) {

	if((temperature >= 298.0) && (temperature <= 300.0) && (pressure <= 500.0)) {

		Output::out("INPUT: CH4 fugacity calculation using BACK EoS\n");
		return ch4_fugacity_back(temperature, pressure);

	}
	else if((temperature == 150.0) && (pressure <= 200.0)) {

		Output::out("INPUT: CH4 fugacity calculation using Peng-Robinson EoS\n");
		return ch4_fugacity_PR(temperature, pressure);

	} 
	else {

		Output::out("INPUT: Unknown if CH4 fugacity will be correct at the requested temperature & pressure...defaulting to use the BACK EoS.\n");
		return ch4_fugacity_back(temperature, pressure);
...
double Fugacity::ch4_fugacity_back(double temperature, double pressure) {
...
double Fugacity::ch4_comp_back(double temperature, double pressure) {

	// Constants
	constexpr double MWCH4          =  16.043;
	constexpr double BACK_CH4_ALPHA =   1.000;
	constexpr double BACK_CH4_U0    = 188.047;
	constexpr double BACK_CH4_V00   =  21.532;
	constexpr double BACK_CH4_N     =   2.40;
	constexpr double BACK_C         =   0.12;
	const int        BACK_MAX_M     =   9;
	const int        BACK_MAX_N     =   4;
...
double Fugacity::ch4_fugacity_PR(double temperature, double pressure) {

	double Z, A, B;
	double alpha, alpha2, Q, X, j, k, l;
	double theta, Q3;
	...
	const double TcCH4 = 190.564;   // K 
	const double PcCH4 = 45.391;    // atm
	const double wCH4  = 0.01142;
	const double R     = 0.08206;   // gas constant atmL/moleK 
	
	const double aa = (0.45724*R*R*TcCH4*TcCH4) / PcCH4;
	const double bb = (0.07780*R*TcCH4) / PcCH4;
	const double Tr = temperature / TcCH4;
...
double Fugacity::n2_fugacity(double temperature, double pressure) {

	if((temperature == 78.0) && (pressure <= 1.0)) {
	
		Output::out("INPUT: N2 fugacity calculation using Zhou\n");
		return n2_fugacity_zhou(temperature, pressure);
	}
	else if((temperature == 78.0) && (pressure >= 10.0) && (pressure <= 300.0)) {
	
		Output::out("INPUT: N2 fugacity calculation using Peng-Robinson EoS\n");
		...
		Output::out("INPUT: N2 fugacity calculation using BACK EoS\n");
		...
		Output::out( "INPUT: Using PR EoS to calculate N2 Fugacity.\nINPUT: Correct N2 fugacity at the requested temperature and pressure has NOT been asserted.\n");
		return n2_fugacity_PR(temperature, pressure);
	}
}
...
// Incorporate BACK EOS 
double Fugacity::n2_fugacity_back(double temperature, double pressure) {
...
double Fugacity::n2_comp_back(double temperature, double pressure) {

	// Constants
	const double BACK_N2_ALPHA =   1.048;
	const double BACK_N2_U0    = 120.489;
	const double BACK_N2_V00   =  18.955;
	const double BACK_N2_N     =  10.81;
	const double BACK_C        =   0.12;
...
double Fugacity::n2_fugacity_PR(double temperature, double pressure) {
...
double Fugacity::n2_fugacity_zhou(double temperature, double pressure) {

	double fugacity_coefficient, fugacity;
	
	Output::out("INPUT: N2 fugacity calculation using Zhou function\n");
...
double Fugacity::co2_fugacity(double temperature, double pressure) {

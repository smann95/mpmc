...
public:
	...
	template<typename T>
	static void calloc( T &ptr, int qty, size_t size, int line, const char *file )
	{
		...
		if( (qty * size) <= 0 ) {	
			sprintf(linebuf, "[internal] Requested %ld bytes: [%d] %s", qty * size, line, file );
			Output::err(linebuf);
		...
		if (ptr == nullptr) {
			sprintf(linebuf, "[runtime system] Failed to allocate %lu bytes: [%d] %s", size * qty, line, file);
			Output::err(linebuf);
	...
	static void malloc( T &ptr, size_t qty, int line, const char *file )
	{
		...
		if( qty <= 0 ) {	
			sprintf(linebuf, "[internal] Requested %ld bytes: [%d] %s", qty, line, file );
			Output::err(linebuf);
			throw memory_request_invalid;
		}
		...
		// Check the allocation
		if( ptr == nullptr ) {
			sprintf(linebuf, "[runtime system] Failed to allocate %lu bytes: [%d] %s", qty, line, file );
			Output::err(linebuf);
			throw memory_request_fail;
		}
	}

...
public:
	...
	typedef struct _observables {
		double energy,
		       ...
		       frozen_mass, total_mass; //updated in average.c
	} observables_t;
...
public:
	...
	int         parallel_restarts; // is this a restart of a parallel job?
	...
	int            feynman_hibbs, 
	               feynman_kleinert,
	               feynman_hibbs_order,
	               vdw_fh_2be,          //2BE method for polarvdw 
	               rd_lrc, 
	               rd_crystal,
	               rd_crystal_order;
	...
	int            polarization, 
	               ...
	               cdvdw_sig_repulsion,
	               cdvdw_9th_repulsion,
	               iterator_failed,                     //flag set when iterative solver fails to converge (when polar_precision is used)
	               polar_iterative, 
	               polar_ewald,
	               polar_ewald_full,
	               polar_zodid,
	...
	int            polar_wolf_alpha_table_max,       //stores the total size of the array
	               damp_type;
	double      ** A_matrix,                         // A matrix, B matrix and polarizability tensor 
	            ** B_matrix,
	               C_matrix[3][3];
	...
	Molecule    ** insertion_molecules_array; // array providing direct access to elements of above list

...
// Backup the observables for two systems (intended for use in Gibbs ensemble)
void System::backup_observables( std::vector<System*> &sys ) {
	int nSystems = sys.size();
	for (int i = 0; i < nSystems; i++)
		memcpy( sys[i]->checkpoint->observables, sys[i]->observables, sizeof(System::observables_t));
}
			...
			for (int s = 0; s < 2; s++)
			{
				if (sys[s]->checkpoint->movetype == MOVETYPE_INSERT)
				{
					...
					if (sys[s]->cavity_bias && sys[s]->cavities_open) {
						...
						for (int i = 0, cavities_array_counter = 0; i < sys[s]->cavity_grid_size; i++) {
					...
					// give it a random orientation 
					sys[s]->checkpoint->molecule_backup->rotate_rand_pbc(1.0, sys[s]->pbc, &sys[s]->mt_rand);
...
void System::boltzmann_factor_NVT_Gibbs(System &sys1, double initEnergy_1, double finalEnergy_1, System &sys2, double initEnergy_2, double finalEnergy_2 ) {
	...
	if( (sys[0]->checkpoint->movetype == MOVETYPE_INSERT   &&   sys[1]->checkpoint->movetype == MOVETYPE_REMOVE)   ||   (sys[0]->checkpoint->movetype == MOVETYPE_REMOVE   &&   sys[1]->checkpoint->movetype == MOVETYPE_INSERT) ) {
		...
		double VA = sys[A]->observables->volume;
		double NA = sys[A]->observables->N;
		double VB = sys[B]->observables->volume;
		double NB = sys[B]->observables->N;
		double Beta = 1.0 / sys[0]->temperature;

		sys[0]->nodestats->boltzmann_factor =
		sys[1]->nodestats->boltzmann_factor = (NA / VA)*(VB / (NB + 1)) * exp(-Beta*dEnergy[A] - Beta*dEnergy[B]);

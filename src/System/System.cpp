...
static constexpr double ewald_alpha_default              = 0.5;
...
static constexpr double wolf_alpha_lookup_cutoff_default = 30.0; //angstroms
...
//	ptemp_t *ptemp;
	...
	preset_seeds[0]               = sd.preset_seeds[0];
	preset_seeds[1]               = sd.preset_seeds[1];
	preset_seeds[2]               = sd.preset_seeds[2];
	preset_seeds[3]               = sd.preset_seeds[3];
	...
	iterator_failed                  = sd.iterator_failed; //flag set when iterative solver fails to converge (when polar_precision is used)
...
bool System::setup_simulation_box()
{
	...
	if(   ! molecules  ) {
		if( ensemble == ENSEMBLE_REPLAY ) {
			...
			Output::err( "SYSTEM: error reading in input molecules\n" );
			throw empty_system;
			return fail;
		}
	}
...
void System::rebuild_arrays () {
	...
	//allocate the arrays
	SafeOps::calloc( molecule_array, n, sizeof( Molecule *), __LINE__, __FILE__ );
	SafeOps::calloc( atom_array,     n, sizeof( Atom *    ), __LINE__, __FILE__ );
...
void System::pairs() {
	...
	for( int i = 0; i < (n - 1); i++) {
		...
		for( int j = (i + 1); j < n; j++ ) {
			...
			// recalc min image
			if( !pair_ptr->frozen || polarization ) //need induced-induced interaction for frozen atoms
...
// computes and updates molecular center of mass 
...
void System::thole_resize_matrices() {
	...
	SafeOps::calloc( A_matrix, N, sizeof(double*), __LINE__, __FILE__ );
	
	for (i=0; i< N; i++ ) 
		SafeOps::malloc( A_matrix[i], N * sizeof(double), __LINE__, __FILE__ );

	// (RE)allocate the B matrix if not iterative
	if( ! polar_iterative ) {
		SafeOps::calloc( B_matrix, N, sizeof(double*), __LINE__, __FILE__ );
		
		for (i=0; i< N; i++ ) 
			SafeOps::malloc(B_matrix[i], N*sizeof(double), __LINE__, __FILE__ );
	}
